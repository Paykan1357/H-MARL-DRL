...
from PortfolioNoCash import PortfolioNoCash as Portfolio  # ✅ بدون نقدینگی
...
CHECKPOINT_BASE = "checkpoints_ablation_no_cash"
RESULTS_BASE = "results_ablation/no_cash"
...
def train_single_seed(seed, args):
    ...
    print(f"\n🚀 Training (No Cash) with seed={seed} on device={device}")
    ...
    for year_idx, year in enumerate(args.train_years):
        ...
        basket = baskets[year]
        print(f"\n{'='*60}")
        print(f"📅 EPOCH {year_idx+1}/{len(args.train_years)}: Trading Year {year} (No Cash)")
        ...
        # MAC Agent بدون نقدینگی: action_dim = n_sectors (بدون +1)
        mac_agent = MACSACAgent(mac_state_dim, len(sector_map), args)
        mac_agent.action_dim = len(sector_map)  # ✅ بدون نقدینگی
        ...
        print(f"✅ Initialized MAC SAC Agent (state={mac_state_dim}, action={len(sector_map)})")
        ...
        while not done:
            try:
                ...
                # ✅ بدون نقدینگی: mac_action فقط شامل آلفاهاست
                
                flat_action = np.concatenate([
                    mac_action,  # بدون cash
