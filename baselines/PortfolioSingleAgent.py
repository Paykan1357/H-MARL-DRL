...
"""
PortfolioSingleAgent.py
نسخه‌ی ساده‌شده برای تک‌عاملی (فقط وزن سهام - بدون بخش و نقدینگی)
"""

import numpy as np
import pandas as pd

class PortfolioSingleAgent:
    def __init__(self, config, stock_names, sector_map, returns_df, features_df, vix_series, year):
        ...
        self.sector_map = sector_map  # فقط برای سازگاری
    ...
    def step(self, flat_action):
        # flat_action: مستقیماً وزن سهام (جمع = ۱)
        final_weights = np.array(flat_action, dtype=np.float32)
        if final_weights.shape[0] != self.num_stocks:
